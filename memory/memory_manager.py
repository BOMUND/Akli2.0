...
def should_extract_memory(user_text: str, assistant_text: str, api_key: str = "") -> bool:
    # Быстрый stage-1 фильтр: стоит ли вообще дёргать LLM-экстрактора.
    try:
...
def extract_memory(user_text: str, assistant_text: str, api_key: str = "") -> dict:
    # Stage-2: вытаскиваем структурированные факты о пользователе в JSON.
    try:
        from or_client import client
        ...
        raw = client.chat(
            ...
            f"                  (e.g. portfolio_site: 'Building a personal portfolio')\n"
            ...
            f' "projects":{{"portfolio_site":{{"value":"personal portfolio"}}}},\n'
